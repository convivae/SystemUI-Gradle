...
EXPECTED_ENTRIES = {
    ...
    # Task 074 (C4c): displaylib kapt half (dagger-generated classes in
    # android_common/javac/, disjoint from the Kotlin half).
    "displaylib-kapt": (
        "displaylib",
        "libs/displaylib-kapt.jar",
    ),
    # Task 074 (C4c): bubbles-user-model (17 bp WindowManager-Shell-defaults
    # static_libs L114; wmshell AAR bytecode references BubbleUserInfo).
    "bubbles-user-model": (
        "bubbles-user-model",
        "libs/bubbles-user-model.jar",
    ),
    "usertypelib": (
        "usertypelib",
...
class TestFrozenMapping(unittest.TestCase):
    def test_mapping_covers_exactly_the_frozen_artifacts(self):
        ...
        self.assertEqual(len(module.CONFIGS), 24)
    ...
    def test_task074_entries_are_frozen_soong_implementation_outputs(self):
        # Task 074 (C4c): bubbles-user-model points at the bubbles-user-model
        # kotlin implementation jar, frozen with source == baseline.
        entry = module.CONFIGS["bubbles-user-model"]
        parts = entry["relpath"].split("/")
        self.assertIn("bubbles-user-model", parts)
        self.assertIn("kotlin", parts)
        self.assertNotIn("kotlin_headers", parts)
        self.assertIn("android_common", parts)
        self.assertEqual(entry["source_sha256"], entry["baseline_sha256"])

    def test_displaylib_kapt_is_a_dagger_component_subset_extraction(self):
        # Task 074 (C4c): displaylib-kapt is a deterministic subset of the
        # displaylib javac (kapt dagger) jar: only the three
        # DaggerDisplayLibComponent classes (the five *_Factory classes are
        # regenerated by :SystemUI-application KSP — R4 duplicate-class
        # evidence). The entry must carry include_prefixes, and its baseline
        # is the extracted artifact fingerprint (not the Soong source).
        entry = module.CONFIGS["displaylib-kapt"]
        parts = entry["relpath"].split("/")
        self.assertIn("displaylib", parts)
        self.assertIn("javac", parts)
        self.assertNotIn("turbine", parts)
        self.assertIn("android_common", parts)
        self.assertIn(
            "com/android/app/displaylib/DaggerDisplayLibComponent",
            entry["include_prefixes"],
        )
        self.assertNotEqual(entry["source_sha256"], entry["baseline_sha256"])

    def test_extract_subset_is_deterministic_and_exact(self):
        # The subset extraction must produce exactly MANIFEST + the three
        # DaggerDisplayLibComponent classes, byte-identical across runs,
        # with the fixed wrapper timestamp.
        import zipfile

        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "javac" / "displaylib.jar"
            source.parent.mkdir()
            payload = b"\xca\xfe\xba\xbe"
            with zipfile.ZipFile(source, "w") as archive:
                archive.writestr("META-INF/MANIFEST.MF", b"Manifest-Version: 1.0\r\n")
                archive.writestr("com/android/app/displaylib/DaggerDisplayLibComponent.class", payload)
                archive.writestr("com/android/app/displaylib/DaggerDisplayLibComponent$Factory.class", payload + b"F")
                archive.writestr("com/android/app/displaylib/DaggerDisplayLibComponent$DisplayLibComponentImpl.class", payload + b"I")
                archive.writestr("com/android/app/displaylib/PerDisplayInstanceRepositoryImpl_Factory.class", payload + b"X")
                archive.writestr("com/android/app/displaylib/DisplayRepositoryImpl_Factory.class", payload + b"Y")
            first = root / "first.jar"
            second = root / "second.jar"
            module._extract_subset(
                source, first, ["com/android/app/displaylib/DaggerDisplayLibComponent"]
            )
            module._extract_subset(
                source, second, ["com/android/app/displaylib/DaggerDisplayLibComponent"]
            )
            ...
            with zipfile.ZipFile(first) as archive:
                ...
                self.assertNotIn(
                    "com/android/app/displaylib/PerDisplayInstanceRepositoryImpl_Factory.class",
                    names,
                )
                self.assertEqual(
                    archive.read("com/android/app/displaylib/DaggerDisplayLibComponent.class"),
                    payload,
                )
                info = archive.getinfo("META-INF/MANIFEST.MF")
                self.assertEqual(info.date_time, module._FIXED_ZIP_TIME)
    ...
    def test_no_entry_is_frozen_as_diff(self):
        ...
        # byte-copy entry must have source == baseline — a mismatch means
        # someone introduced a new hand-copied jar. Subset-extraction entries
        # (include_prefixes) are exempt: their baseline is the extracted
        # artifact, by design distinct from the Soong source fingerprint.
        diff = {
            name
            for name, entry in module.CONFIGS.items()
            if entry["source_sha256"] != entry["baseline_sha256"]
            and not entry.get("include_prefixes")
        }
        self.assertEqual(diff, set())
        # The only extracted-subset entry is displaylib-kapt (Task 074).
        self.assertEqual(
            {
                name
                for name, entry in module.CONFIGS.items()
                if entry.get("include_prefixes")
            },
            {"displaylib-kapt"},
        )
