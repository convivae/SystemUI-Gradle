...
> **For the architect:** Execute via the herdr orchestration workflow (orchestrator skill). Each fix task below is dispatched as a worker brief copied into `docs/orchestration/tasks/NNN-<slug>.md`. Steps use checkbox (`- [ ]`) syntax.
...
- [ ] 1. After Tasks 1–5 merge, run the full gate:
...
- [ ] 2. Record the truthful outcome: remaining javac error count by group (expected: only the `NeverCompile` group may remain, pending the research decision), or the next failing task if the build advanced past javac. No success claims without the log.
- [ ] 3. Sync the maintained docs with the outcome (same edit set as the Task 7 documentation pass); append the wave summary to `docs/orchestration/log.md`; mark this plan's checkboxes.
- [ ] 4. Commit and push:
