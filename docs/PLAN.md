...
- [ ] **C5：17 镜像双 runtime 门**：task075 Debug 热运行、task076 Release protobuf-lite、task077 durable emulator、task078/080 四条 exact mappings 与 166 caller identities均已闭合；Task 081 reference-only build logic已通过 focused tests/双轴review。Task 082真实Debug pipeline暴露AGP factory isolation；Task 093把完整transient cache layer固定为当前最小已知activation boundary，**Task 094现已正式`PASS`：配置阶段校验的4/166 managed values + field-free no-op factory通过真实dependency-transform isolation。下一步Task 095把该seam迁入production、恢复visitor并完成9个focused tests、两个external-file output hidden refs 0/2→2/2 direct proof及双轴review；之后依次执行Debug build/static、Release build/static、Debug runtime、Release runtime。** Task 079 broad replay保持暂停。
...
是当前最小已知activation boundary，但不证明其中单个字段、annotation、accessor或writeback为sole trigger。Task 094随后正式`PASS`：configuration-time validated immutable managed values + field-free no-op factory的唯一direct control exit 0，三个sentinel各1、45条ASM records，known serialization markers为0。该结果只证明isolation-safe seam，不证明production visitor。Task 095现已设计：迁移production seam、恢复`referenceOnlyVisitor(...)`、运行9个focused tests，并要求两个external-file DEX outputs的hidden target refs从0/2变为2/2且hidden definitions保持0/2；之后完成Chief验收与双轴review。APK尚未重编，Task 079 broad replay暂停，模拟器当前未运行。最新证据见
