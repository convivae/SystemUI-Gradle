...
4. Task 083 is the next serialized rung: one read-only worker runs only direct `:app:desugarDebugFileDependencies --stacktrace`, proves the deepest isolation cause against AGP/Gradle source and current classfile flags, and proposes—but does not implement—the minimum regression gate/fix. A fresh Debug build/static gate remains separate; Release/R8 and device work remain forbidden until Debug succeeds.
