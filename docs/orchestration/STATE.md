...
- 2026-09-02 — Task 097 fresh Release build/R8/static gate designed after Task 096 durable closure. It is no-fix and permits exactly one `:app:assembleRelease --rerun-tasks` call after deleting only the stale generated Release APK. PASS requires actual R8/package execution, fresh APK ZIP/SHA evidence, checker exit 0 / `RESULT=PASS`, critical old references/definitions 0/4, critical hidden references 4/4, hidden definitions 0/4, and all-725 hidden target definitions 0. Debug, devices, runtime, repair work, network/ref mutation, and Task 079 are excluded. Dispatch waits for the planning documents to be committed and pushed.
