...
14. Task 095 is designed but not dispatched. It migrates the proven managed-value seam into production, removes factory cache/state, restores `referenceOnlyVisitor(...)`, runs the nine focused tests, and requires two known external-file DEX outputs to change hidden-target refs from 0/2 to 2/2 with zero hidden definitions. Full builds, Release/R8, device work, and Task 079 remain excluded.
