...
19. Task 098 is planned as the independent fresh Debug runtime reboot gate. It consumes only Task 096 APK `f3af35d9…`, launches the Task 077 durable emu64x environment, and requires exact host/device SHA plus two separate 11×30s PID/log/UI windows around an explicit whole-device reboot. No build or repair is allowed; Release runtime remains a later task.
20. The first Task 098 worker `task098-debug-runtime` was retired before preflight because it opened the task brief before the frozen startup sequence and did not reread it at the mandated position. Session identity was correct, but its CONTRACT was not accepted. It performed documentation reads only: no scratch, emulator/ADB/build/process action, or tracked change. A replacement awaits a new clean pushed dispatch base.
