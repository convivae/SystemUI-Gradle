...
12. Task 094 is designed but not yet dispatched. The selected seam replaces factory-owned cache state with a configuration-time validated immutable snapshot stored in Gradle-managed `MapProperty<String, String>` and `SetProperty<String>` inputs; the control factory remains field-free and the visitor byte-no-op.
13. Task 094 may run exactly one `:app:desugarDebugFileDependencies` command and must classify direct sentinels/ASM execution versus the Task 084/093 literal path. It cannot claim production visitor, APK, R8, checker, or runtime success. If it passes, production seam migration and visitor proof remain a separate task; Task 079 remains paused.
