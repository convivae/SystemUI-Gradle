...
优化 Release 主分支当前是 **109 个 R8 missing refs**。Task 035 已完成双轴 PASS、合并和主分支 fresh 复验：精确移除 11 个计划的 program refs，并新暴露一个设备提供的 B2 `org.apache.harmony.dalvik.ddmc.ChunkHandler` library ref；用户已接受该真实结果并将其延期到 B2 bridge。后续继续 A 类 Batch 4 闭包，再处理 B1–B4 library/classpath closure；APK 装机/运行验证未做。

## Last Updated

2026-08-20 — Task 035 merged as `bf6ff75f` after independent Standards+Spec PASS. Main fresh verification: 6 focused and 160 total tests pass; deterministic clean JAR hashes/classes match; debug duplicate/build succeeds; after clearing stale incremental APK ZIP data the V2-signed APK is 160,547,785 B and all five representative classes are defined. Fresh R8 accurately yields 119→109: exactly 11 planned refs removed, exactly one B2 `org.apache.harmony.dalvik.ddmc.ChunkHandler` added, and `AssumeTrueForR8` retained. An initial all-task rerun was killed by kernel OOM because Gradle (~12.6 GiB) overlapped an orphan Kotlin daemon (~10.8 GiB); after stopping the orphan, the standard R8 command reached the expected missing-class failure. Next: continue Batch 4 A-class closure, then B1–B4 library bridge work. All waits remain capped at 90 seconds.
