...
8. Task 092 is the next serial diagnostic: retain the proven parameter/load rung, positively admit `android.os.CustomFeatureFlags` through the production allowlist helper, and observe a byte-no-op visitor. Cache state and `referenceOnlyVisitor(...)` remain forbidden.
