...
**Status**: proposed → completed in `1f5e93e8`; chief review returned FAIL (fabricated R8-library claim, ownership-unsafe algorithm, sharding misattribution, 726-rule wording, gate robustness); corrective commit `b4e021e8` applied the six required fixes; second chief review returned FAIL again (wrong AGP API name, per-module scope misconception, gate could PASS by shipping hidden definitions, over-generalized source-absence criterion, broken transform fixture, brief pretended known implementation paths); corrective commit `cb1223f4` applied the five required fixes; dual-axis review PASS; final acceptance found one MEDIUM docs-only contract contradiction (E4 was defined as actual-R8-dependent in §4.1/§4.5 while §5.1 forbade every actual R8 path and downgraded E4 to static class-existence) — docs-only corrective commit redefines E4 as a standalone invocation of the AGP 9.3.1 bundled R8 (`builder-9.3.1.jar`, reports R8 9.3.16; not a Gradle task) with scratch probe artifact, SysUISdk `--lib`, negative control against `android-37.0`, and synchronizes report/issue/brief. Code and tests unchanged; E1–E4 remain not run in Task 078. Implementation of the selected rewrite remains out of scope pending E1–E4 experiments and user adjudication.
...
- [x] Draft a separate implementation brief with exact allowed paths, red lines, tests, and rollback criteria. Do not execute it. (Second review: the next executable brief now covers only the E1–E4 experiment task with its own Allowed/Forbidden Paths and gates — report §5.1; implementation Allowed Paths and the affected-artifact coordinate list are explicitly deferred until E1 outputs the complete inventory — report §5.2. Final acceptance correction: E4 within that brief is an actual R8 run — a standalone `java -cp builder-9.3.1.jar com.android.tools.r8.R8` invocation, not a Gradle task — with a scratch probe artifact referencing all four critical hidden targets, SysUISdk android.jar as `--lib`, shrinking/minification/desugaring disabled, and an `android-37.0` negative control where feasible)
